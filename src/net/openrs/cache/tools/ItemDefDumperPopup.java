...
public class ItemDefDumperPopup {
	...
	public static final String EXPORT = "C:/Users/Stan/Desktop/RS/osrscache/type/";
	...
	public static void main(String[] args) throws Exception {
		...
		if (!bonusses.isEmpty()) {
			...
			oldBonusses = JOptionPane.showConfirmDialog(null, "Do you want the old bonusses?", "Old bonusses",
					dialogButton) == 0;
		}
		...
		try (BufferedReader br = new BufferedReader(new FileReader(LISTFILE))) {
			...
			while (!finished) {
				...
				if (line.startsWith("case ")) {
					...
					if (item.name.contains(" arrow") || item.name.contains(" knife") || item.name.contains(" dart")
					...
					if (itemname.contains("steel")) {
						if ((itemname.contains("sword")) || (itemname.contains("scimitar"))
								|| (itemname.contains("battleaxe")) || (itemname.contains("warhammer"))
								|| (itemname.contains("spear")) || (itemname.contains("dagger"))) {
							item.requirements[0] = 5;
						} else if ((itemname.contains("platebody")) || (itemname.contains("chainbody"))
								|| (itemname.contains("boots")) || (itemname.contains("legs"))
								|| (itemname.contains("skirt")) || (itemname.contains("gloves"))
								|| (itemname.contains("shield")) || (itemname.contains("defender"))) {
							item.requirements[1] = 5;
						} else if ((itemname.contains("knife")) || (itemname.contains("thrownaxe"))
								|| (itemname.contains("javelin")) || (itemname.contains("dart"))) {
					...
					if ((itemname.contains("berserker helm")) || (itemname.contains("farseer helm"))
							|| (itemname.contains("archer helm")) || (itemname.contains("warrior helm"))) {
					...
					try {
						...
						if (item.name.contains("Burnt ")) {
							...
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.replaceAll(" ", "_").substring(0, item.name.length() - 4);
							document = openDocument(url);
						} else if (item.name.contains("Ahrim") || item.name.contains("Dharok")
								|| item.name.contains("Guthan") || item.name.contains("Karil")
								|| item.name.contains("Torag") || item.name.contains("Verac")) {
							if (item.name.contains("100")) {
								url = "http://2007.runescape.wikia.com/wiki/"
										+ item.name.replaceAll(" ", "_").substring(0, item.name.length() - 4);
							} else {
								url = "http://2007.runescape.wikia.com/wiki/"
										+ item.name.replaceAll(" ", "_").substring(0, item.name.length() - 3);
								item.forceUntradable = true;
							}
							document = openDocument(url);
						} else if (item.name.contains("- pt")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.substring(0, item.name.length() - 6).replaceAll(" ", "_");
							document = openDocument(url);
						} else if (item.name.contains("(m1)") || item.name.contains("(m2)")
								|| item.name.contains("(m3)") || item.name.contains("(m4)")
								|| item.name.contains("(10)")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.substring(0, item.name.length() - 4).trim().replaceAll(" ", "_");
							document = openDocument(url);
						} else if ((item.name.contains("(0)") || item.name.contains("(1)") || item.name.contains("(2)")
								|| item.name.contains("(3)") || item.name.contains("(4)") || item.name.contains("(5)")
								|| item.name.contains("(6)") || item.name.contains("(7)") || item.name.contains("(8)")
								|| item.name.contains("(9)") || item.name.contains("(o)"))
								&& !item.name.contains("Mixture")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.substring(0, item.name.length() - 3).trim().replaceAll(" ", "_");
							try {
								document = openDocument(url);
							} catch (HttpStatusException ex) {

							}
						} else if ((item.name.endsWith("0") || item.name.endsWith("1") || item.name.endsWith("2")
								|| item.name.endsWith("3") || item.name.endsWith("4") || item.name.endsWith("5")
								|| item.name.endsWith("6") || item.name.endsWith("7") || item.name.endsWith("8")
								|| item.name.endsWith("9") || item.name.endsWith("10"))
								&& !item.name.contains("Magical balance") && !item.name.contains("Rune case")
								&& !item.name.contains("Godsword shards")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.substring(0, item.name.length() - 2).trim().replaceAll(" ", "_");
							document = openDocument(url);
						} else if (item.name.endsWith("cape(t)")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.substring(0, item.name.length() - 3).trim().replaceAll(" ", "_")
											.replaceAll("Woodcut.", "Woodcutting");
							document = openDocument(url);
						} else if (item.name.contains("hat and")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.replaceAll("hat and", "hat %26").replaceAll(" ", "_");
							document = openDocument(url);
						} else if (item.name.contains(" (uncharged)")) {
							url = "http://2007.runescape.wikia.com/wiki/"
									+ item.name.substring(0, item.name.length() - 12).replaceAll(" ", "_");
							document = openDocument(url);
					...
					if (document != null) {
						BufferedWriter htmlWriter = new BufferedWriter(
								new OutputStreamWriter(new FileOutputStream("C:/Users/Stan/Desktop/RS/wiki/"
										+ url.replace("http://2007.runescape.wikia.com/wiki/", "").replace("/", "")
												.replace("???", "")
										+ ".html"), "UTF-8"));
						...
						// Files.copy(in, Paths.get("C:/Users/Stan/Desktop/Item sprites/" + item.id +
						// ".png"));
						// } catch(IOException e) {
						...
						for (org.jsoup.nodes.Element infoBox : list) {
							...
							if (examineNext) {
								...
								if (text.contains("Members only")) {
									...
									if (gemain.text().contains("dose")) {
										if (item.name.contains("4")) {
											item.geprice = Integer
													.parseInt(ge.get(ge.size() - 4).text().replace(",", ""));
										} else if (item.name.contains("3")) {
											item.geprice = Integer
													.parseInt(ge.get(ge.size() - 3).text().replace(",", ""));
										} else if (item.name.contains("2")) {
											item.geprice = Integer
													.parseInt(ge.get(ge.size() - 2).text().replace(",", ""));
										} else if (item.name.contains("1")) {
											item.geprice = Integer
													.parseInt(ge.get(ge.size() - 1).text().replace(",", ""));
										}
									} else if (ge.first() != null && !ge.first().text().contains("error")) {
										item.geprice = Integer
												.parseInt(ge.first().text().replace(",", "").replaceAll("Clean:", ""));
									}
								} else if (text.contains("Equipable")
										&& !text.contains("if the player does not unwield it ")
										&& !item.name.contains("Fishing bait")) {
									item.equipable = true;
										...
										if (bonusIndex < 14) {
											try {
												if (bonusNumber.text().contains("to")) {
													item.bonusses[bonusIndex] = Integer
															.parseInt(bonusNumber.text().replaceAll("%", "")
																	.replaceAll("\\(+4 trimmed\\)", "").split("to")[1]
																			.trim().replaceAll("\u00A0", ""));
												} else {
													item.bonusses[bonusIndex] = Integer.parseInt(bonusNumber.text()
															.replaceAll("%", "").replaceAll("\\(+4 trimmed\\)", ""));
												}
												bonusIndex++;
									...
									if (slotElement.last() != null) {
										...
										case "Head":
											item.slot = 0;
											if (item.name.toLowerCase().contains("helm")
													&& item.name.toLowerCase().contains("full")) {
												item.helm = true;
										...
										case "Weapon":
											item.slot = 3;
											if (document.select(".wikitable.smallpadding tr th span img").last()
													.attr("alt").contains("Monster")) {
												item.speed = Integer.parseInt(document
														.select(".wikitable.smallpadding tr th span img").last()
														.attr("alt").replace("Monster attack speed", "").trim());
											}
											break;
										case "Two-handed":
										case "2h":
										...
										case "Body":
											...
											if (item.name.toLowerCase().contains("body")
													...
													|| item.name.toLowerCase().contains("robe")
													|| item.name.toLowerCase().contains("robetop")
													|| item.name.toLowerCase().contains("chestplate")) {
												item.body = true;
											}
											break;
										case "Shield":
										...
										case "Ring":
											...
											break;
										default:
											System.out.println("SLOT NIET BEKEND - "
													+ slotElement.last().attr("title").replaceAll(" slot table", ""));
											break;
										}
									}
									...
									if (descr != "" && !(descr.toLowerCase().contains("enchant")
											&& (item.slot == 2 || item.slot == 12))) {
										descr = "to prevent errors boiiis" + descr + "to prevent errors boiiis";
										String reqText = "";
										String extraText = "";
										if (descr.contains("attack")) {
											extraText = descr.substring(descr.indexOf("attack") - 22,
													descr.indexOf("attack") + 26);
											...
											if (descr.contains("attack")) {
												...
												if (descr.contains("attack")) {
													extraText = descr.substring(descr.indexOf("attack") - 22,
															descr.indexOf("attack") + 26);
													if (extraText.matches(".*\\d+.*")
															&& !extraText.contains("jinxed")) {
														reqText += extraText;
													}
													descr = descr.replaceFirst("attack", "jinxed");
												}
										...
										if (descr.contains("defence")) {
											extraText = descr.substring(descr.indexOf("defence") - 22,
													descr.indexOf("defence") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
										...
										if (descr.contains("strength")) {
											extraText = descr.substring(descr.indexOf("strength") - 22,
													descr.indexOf("strength") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
										...
										if (descr.contains("hitpoints")) {
											extraText = descr.substring(descr.indexOf("hitpoints") - 22,
													descr.indexOf("hitpoints") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
										...
										if (descr.contains("magic")) {
											extraText = descr.substring(descr.indexOf("magic") - 22,
													descr.indexOf("magic") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
										...
										if (descr.contains("ranged")) {
											extraText = descr.substring(descr.indexOf("ranged") - 22,
													descr.indexOf("ranged") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
										...
										if (descr.contains("prayer")) {
											extraText = descr.substring(descr.indexOf("prayer") - 22,
													descr.indexOf("prayer") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
										...
										if (descr.contains("agility")) {
											extraText = descr.substring(descr.indexOf("agility") - 22,
													descr.indexOf("agility") + 26);
											if (extraText.matches(".*\\d+.*") && !extraText.contains("jinxed")) {
												reqText += extraText;
											}
							...
							} else if (text.contains(" Alch") && !text.contains("Varies") && !text.contains("unknown")
									&& !text.contains("Various") && !text.contains("N/A") && !text.contains("herbs")) {
								text = infoBox.children().last().text();
								if (text.contains("doses")) {
									text = text.split("doses: ")[1];
									if (text.split(",").length > 1) {
										text = text.split(",")[Integer.parseInt(
												item.name.substring(item.name.length() - 2, item.name.length() - 1))
												- 1];
									}
								} else if (text.contains("Whole")) {
									text = text.split("Half")[0].split("Whole:")[1];
								} else if (text.contains("dose =")) {
									text = infoBox.children().last().text().replaceAll("dose", "")
								...
								if (infoBox.text().contains("High Alch") && !text.contains("Unknown")
										&& !text.contains("?")) {
									item.highAlch = Integer
											.parseInt(text.split("coins")[0].split("coin")[0].split("\\(")[0]
													.replaceAll("Clean:", "").replaceAll("coins", "")
													.replaceAll("coin", "").trim().replace(",", "")
													.replaceAll("\u00A0", ""));
								} else if (infoBox.text().contains("Low Alch") && !text.contains("Unknown")
										&& !text.contains("?")) {
									item.lowAlch = Integer
											.parseInt(text.split("coins")[0].split("coin")[0].split("\\(")[0]
													.replaceAll("Clean:", "").replaceAll("coins", "")
													.replaceAll("coin", "").trim().replace(",", "")
													.replaceAll("\u00A0", ""));
								...
								if (text.contains("Equipped")) {
									item.weight = Double.parseDouble(
											infoBox.children().last().text().split("Equipped:")[1].split("kg")[0]
													.replaceAll("\u00A0", ""));
								} else if (text.contains("(full)")) {
									item.weight = Double.parseDouble(
											infoBox.children().last().text().split("\\(full\\)")[1].split("kg")[0]
													.replaceAll("\u00A0", ""));
								} else if (text.contains("Whole:")) {
									item.weight = Double.parseDouble(
											infoBox.children().last().text().split("Half")[0].split("Whole:")[1]
													.split("kg")[0].replaceAll("\u00A0", ""));
								} else if (text.contains("Raw:")) {
									item.weight = Double.parseDouble(
											infoBox.children().last().text().split("Cooked")[0].split("Raw:")[1]
													.split("kg")[0].replaceAll("\u00A0", ""));
								} else {
									item.weight = Double.parseDouble(
											infoBox.children().last().text().split(",")[0].split("to")[0].split("kg")[0]
	...
	private static org.jsoup.nodes.Document openDocument(String url) throws IOException {
		File input = new File("C:/Users/Stan/Desktop/RS/wiki/"
				+ url.replace("http://2007.runescape.wikia.com/wiki/", "").replace("/", "").replace("???", "")
				+ ".html");
		if (input.exists()) {
